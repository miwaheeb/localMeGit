...
void puTTy(void)
{
  // if there are is line input
  if(Serial.available() > 0)
  {
    // store char in var response as a upper case letter
    response = toUpperCase(char(Serial.read()));
    Serial.print(response); // print the char response
    serialArray[arrayInc] = response; // store response in array
    arrayInc++; // inc array

    // if response equal \r -> ENTER press
    if(response == '\r')
    {
      //Serial.print("\n\r");
      arrayInc--;
      arrayInc--;
      //Serial.print(serialArray[arrayInc]);

      if(serialArray[arrayInc] == 'T')
      {
        Serial.print("\n\r");
        Serial.print("\n\rTemperature: ");
      ...
      else if(serialArray[arrayInc] == 'H')
      {
        Serial.print("\n\r");
        Serial.print("\n\rRelative humidity: ");
